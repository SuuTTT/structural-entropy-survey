...
It is both a measure of residual structural uncertainty and an optimization
principle for discovering efficient hierarchical decompositions.
That combination has made SE useful not only for community detection, where it
first gained attention, but also for graph pooling, augmentation, clustering,
control, and domain-specific discovery tasks~\cite{wu2022pooling,wu2023sega,zeng2025sidmJMLR,zhang2021supertad,peng2024zjyun,zhu2023HiTIN}.

This perspective is timely for graph intelligence because contemporary systems
increasingly need more than accurate predictions.
They need structural abstractions that are interpretable, reusable, and stable
enough to support retrieval, reasoning, and decision making.
The need is especially visible in graph-enhanced large language model
pipelines, knowledge-graph reasoning systems, tool-use memory, and
autonomous-agent interaction graphs.
These systems create relational structures whose usefulness depends on whether
meaningful hierarchies can be separated from incidental links.
